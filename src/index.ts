...
export function apply(ctx: Context, config: Config) {
  ...
  async function applyStatusTransition(room: RoomConfig, status: BackendStatus): Promise<void> {
    ...
    if (status.is_live) {
      ...
      if (cached) {
        if (!status.cover_url && cached.cover_url) status.cover_url = cached.cover_url
        if (!status.avatar_url && cached.avatar_url) status.avatar_url = cached.avatar_url
      }
    }
  ...
  function cleanupStaleMaps() {
    ...
    for (const key of liveStartedAt.keys()) {
      if (!activeKeys.has(key)) liveStartedAt.delete(key)
    }
