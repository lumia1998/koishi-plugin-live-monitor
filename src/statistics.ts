...
export function formatStatisticsDuration(seconds: number) {
  const minutes = Math.max(0, Math.floor(seconds / 60))
  const days = Math.floor(minutes / 1440)
  const hours = Math.floor((minutes % 1440) / 60)
  const restMinutes = minutes % 60
  if (days) return `${days}天 ${hours}小时`
  if (hours) return `${hours}小时 ${restMinutes}分`
  return `${restMinutes}分钟`
}

export function getStatisticsRange(period: StatisticsPeriod, now = new Date()) {
  if (period === 'day') {
...
export function summarizeSessions(
  sessions: LiveMonitorSessionRecord[],
  rangeStart: Date,
  rangeEnd: Date,
  now = new Date(),
): StatisticsSummary {
  ...
  for (const session of sessions) {
    ...
    if (times.startedAt >= rangeStart && times.startedAt < rangeEnd) {
      ...
      if (startBucket) startBucket.sessionStarts += 1
    }
...
function backgroundLayer(avatarDataUrl?: string) {
  if (!avatarDataUrl) return '<div class="portrait-fallback"></div>'
...
function profileHeader(input: StatisticsCardInput, eyebrow: string) {
...
export function buildCalendarStatisticsHtml(input: StatisticsCardInput, year: number, month: number) {
  const { summary } = input
  ...
  const cells: string[] = []
  for (let i = 0; i < firstDay.getDay(); i++) cells.push('<div class="day empty"></div>')
...
export function getAdaptiveTimeAxis(maxSeconds: number) {
...
function buildLineChart(summary: StatisticsSummary) {
  const width = 710
  const height = 260
  const left = 48
  const right = 12
  ...
  const xLabels = points.map((point, index) => {
    if (index % labelInterval !== 0 && index !== points.length - 1) return ''
...
export function buildPeriodStatisticsHtml(input: StatisticsCardInput) {
  const { summary } = input
...
export function buildOverviewStatisticsHtml(
  input: StatisticsCardInput,
  daySummary: StatisticsSummary,
  weekSummary: StatisticsSummary,
