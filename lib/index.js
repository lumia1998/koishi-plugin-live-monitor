...
function buildLiveCardHtml(status, started, images = {}) {
    ...
    if (started && durationText)
        metaLineItems.push(`直播时长：${durationText}`);
    if (timeText)
        metaLineItems.push(`${started ? '开播时间' : '结束时间'}：${timeText}`);
...
async function captureRoomCover(ctx, page, status) {
    if (!status.url)
        return;
    try {
...
async function prepareLiveCardImages(ctx, page, status) {
