...
function apply(ctx, config) {
    ...
    async function notify(room, status, started, mentionAll = false) {
        ...
        if (image) {
            const content = (0, koishi_1.h)('message', [...mentionPrefix, koishi_1.h.image(image, 'image/png'), koishi_1.h.text(`\n${status.url}`)]);
            await ctx.broadcast(channels, content);
            return;
        }
        await ctx.broadcast(channels, shouldMentionAll ? (0, koishi_1.h)('message', [...mentionPrefix, koishi_1.h.text(message)]) : message);
    }
