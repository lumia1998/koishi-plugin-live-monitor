...
exports.inject = { optional: ['puppeteer'] };
...
exports.Config = koishi_1.Schema.object({
    ...
    notifyChannels: koishi_1.Schema.array(String).role('table').default([]).description('默认通知频道 ID。留空时，未单独绑定频道的直播间会向所有已分配频道推送。'),
    ...
    liveReminderInterval: koishi_1.Schema.number().min(0).default(0).description('正在直播中的主播重复推送提醒间隔（分钟），设为 0 表示不重复推送（仅开/关播时推送）。如设为 30 表示每半小时提醒一次。'),
    rooms: koishi_1.Schema.array(koishi_1.Schema.object({
        ...
        channels: koishi_1.Schema.string().default('').description('绑定频道 ID。多个频道用逗号分隔；留空使用默认通知频道，默认也为空则所有群可见并推送。'),
...
function apply(ctx, config) {
    ...
    async function notify(room, status, started) {
        ...
        if (image) {
            const content = (0, koishi_1.h)('message', [koishi_1.h.image(image, 'image/png'), koishi_1.h.text(`\n${status.url}`)]);
    ...
    ctx.command('live-monitor.status', '发送本群当前开播直播间卡片')
        ...
        for (const status of liveStatuses) {
            ...
            if (image) {
                await session.send((0, koishi_1.h)('message', [koishi_1.h.image(image, 'image/png'), koishi_1.h.text(`\n${status.url}`)]));
            }
            else {
