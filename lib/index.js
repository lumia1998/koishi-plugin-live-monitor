...
exports.buildLiveCardHtml = buildLiveCardHtml;
exports.apply = apply;
const node_crypto_1 = require("node:crypto");
const node_path_1 = require("node:path");
const koishi_1 = require("koishi");
const avatar_cache_1 = require("./avatar-cache");
...
exports.Config = koishi_1.Schema.object({
    ...
    offlineConfirmations: koishi_1.Schema.number().min(1).max(5).step(1).default(2).description('连续检测到未开播多少次后才确认下播。默认 2 次，可避免平台瞬时误判切断直播统计。'),
    notifyOnStart: koishi_1.Schema.boolean().default(true).description('检测到开播时推送'),
    notifyOnEnd: koishi_1.Schema.boolean().default(true).description('检测到关播时推送。需要插件此前检测并记录过该主播开播。'),
...
function roomKey(room) {
    return (0, node_crypto_1.createHash)('sha256').update(normalizeRoomUrl(room.url)).digest('hex');
}
...
async function renderStatisticsCard(ctx, html) {
    const puppeteer = ctx.puppeteer;
    if (!puppeteer)
        return;
    let page;
...
function apply(ctx, config) {
    ...
    const offlineDetectedAt = new Map();
    const avatarCache = new avatar_cache_1.AvatarCache((0, node_path_1.join)(ctx.baseDir || process.cwd(), 'data', 'live-monitor', 'avatars'));
    let checking = false;
    let restorePromise;
                ...
                try {
                    ...
                    for (const session of sessions) {
                        ...
                        if (session.completed)
                            continue;
                        if (!activeKeys.has(session.roomId))
                            continue;
                        if (!previous.has(session.roomId))
                            previous.set(session.roomId, true);
                        const startedAt = parseStoredTime(session.startedAt);
                        if (startedAt !== undefined && !liveStartedAt.has(session.roomId))
                            liveStartedAt.set(session.roomId, startedAt);
                        if (session.id && !activeSessionIds.has(session.roomId))
                            activeSessionIds.set(session.roomId, session.id);
                    }
                    for (const row of rows) {
                        if (!activeKeys.has(row.id))
                            continue;
                        ...
                        if (startedAt !== undefined)
                            liveStartedAt.set(row.id, startedAt);
                        if (notifiedAt !== undefined)
                            lastNotified.set(row.id, notifiedAt);
                        if (row.sessionId)
                            activeSessionIds.set(row.id, row.sessionId);
                    }
    ...
    async function saveLiveSession(key, room, status, endedAt) {
        if (!ctx.database)
            return;
        const startedAt = liveStartedAt.get(key);
        ...
        if (!sessionId) {
            sessionId = (0, node_crypto_1.createHash)('sha256').update(`${key}\x00${new Date(startedAt).toISOString()}`).digest('hex');
    ...
    async function getCachedAvatar(key, avatarUrl, referer) {
    ...
    async function applyStatusTransition(room, status) {
        ...
        if (oldState === true && !status.is_live) {
            ...
            if (!offlineDetectedAt.has(key))
                offlineDetectedAt.set(key, now);
        ...
        if (status.is_live) {
            ...
            if (status.avatar_url)
                await getCachedAvatar(key, status.avatar_url, status.url);
        }
        else {
        ...
        if (status.is_live)
            await saveLiveSession(key, room, status);
    ...
    function selectStatisticsRoom(name, session) {
        const rooms = getEnabledRooms(session);
        if (!rooms.length)
            return { error: '当前群没有绑定或可见的直播监控项。' };
        ...
        const matches = rooms.filter(room => (room.name || '').toLowerCase().includes(query) ||
            normalizeRoomUrl(room.url).toLowerCase().includes(query));
        if (matches.length === 1)
            return { room: matches[0] };
        if (!matches.length)
            return { error: `没有找到主播“${name}”。` };
        return { error: `匹配到多个主播，请输入更完整的名称。` };
    }
    async function getRoomSessions(room) {
    ...
    async function fetchSessionsFromBackend(room) {
        const token = config.apiToken?.trim();
        try {
            const response = await ctx.http.get(`${trimSlash(config.endpoint)}/api/sessions?limit=5000`, token ? { headers: { 'X-API-Token': token } } : undefined);
    ...
    async function buildStatisticsImage(room, period, rangeStart, rangeEnd, calendar) {
        const sessions = await getRoomSessions(room);
        const summary = (0, statistics_1.summarizeSessions)(sessions, rangeStart, rangeEnd);
        ...
        const html = calendar
            ? (0, statistics_1.buildCalendarStatisticsHtml)(input, calendar.year, calendar.month)
            : (0, statistics_1.buildPeriodStatisticsHtml)(input);
        return {
            image: await renderStatisticsCard(ctx, html),
            hasRecords: summary.sessionCount > 0,
        };
    }
    async function buildOverviewStatisticsImage(room) {
        const sessions = await getRoomSessions(room);
        const now = new Date();
        const dayRange = (0, statistics_1.getStatisticsRange)('day', now);
        const weekRange = (0, statistics_1.getStatisticsRange)('week', now);
        const monthRange = (0, statistics_1.getStatisticsRange)('month', now);
        const daySummary = (0, statistics_1.summarizeSessions)(sessions, dayRange.start, dayRange.end, now);
        const weekSummary = (0, statistics_1.summarizeSessions)(sessions, weekRange.start, weekRange.end, now);
        const monthSummary = (0, statistics_1.summarizeSessions)(sessions, monthRange.start, monthRange.end, now);
        const latest = sessions[sessions.length - 1];
        const avatarDataUrl = await getCachedAvatar(roomKey(room), latest?.avatarUrl, latest?.roomUrl || room.url);
        const input = {
            displayName: latest?.displayName || room.name || room.url,
        ...
        return {
            image: await renderStatisticsCard(ctx, (0, statistics_1.buildOverviewStatisticsHtml)(input, daySummary, weekSummary, monthSummary)),
    ...
    ctx.command('live-monitor.calendar [name:text]', '查看主播的月度直播打卡日历')
        ...
        if (!result.image)
            return '直播打卡图生成失败，请查看插件日志。';
        return koishi_1.h.image(result.image, 'image/png');
    });
    ctx.command('live-monitor.stats [name:text]', '查看主播今日、本周和本月的直播统计')
        ...
        if (!result.hasRecords)
            return '该主播本月还没有直播记录。';
        if (!result.image)
            return '直播统计图生成失败，请查看插件日志。';
        return koishi_1.h.image(result.image, 'image/png');
    });
    function registerStatisticsCommand(command, alias, description, period) {
