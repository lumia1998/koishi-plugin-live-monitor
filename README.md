...
- **卡片式提醒**：在支持 Puppeteer 的环境下，可生成开播提醒卡片，并在同一条消息中附带直播地址；卡片会优先展示直播封面、主播头像、标题、人气、分区、点赞和直播时长，没有封面时会尝试截图直播间画面，也可切换为纯文字通知。
...
- **`notifyOnEnd`**：检测到下播时推送，默认开启；需要插件运行期间先检测到该主播开播。若部分平台下播时后端连续两轮返回检测异常且状态为未直播，插件也会按下播处理并推送。
- **`rooms` (关注主播列表)**：
